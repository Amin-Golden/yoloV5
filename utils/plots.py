...
class Annotator:
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            self.draw.rectangle(box, width=self.lw,fill=(0,0,0), outline=color)  # box
            if label:
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=1920, max_subplots=16):
    ...
    for i in range(i + 1):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
        annotator.rectangle([x, y, x + w, y + h], (0, 0, 0), (255, 255, 255), width=2)  # borders
        if paths:
...
def plot_labels(labels, names=(), save_dir=Path('')):
    ...
    for cls, *box in labels[:1000]:
        ImageDraw.Draw(img).rectangle(box, width=1, fill=(0,0,0),outline=colors(cls))  # plot
    ax[1].imshow(img)
    ax[1].axis('off')
